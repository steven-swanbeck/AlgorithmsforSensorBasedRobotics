function [FK_solution, T_bank, T_total_bank] = FK_space2(M, S_mat, thetas, should_plot, M_intermediates, alpha, show_axes, should_create_plot)
...
%   false and the output will be plotted on the current figure in focus.
    ...
    if should_plot == true   
        ...
        plotFrame_T(eye(4), '$\lbrace s \rbrace$', 0.1, alpha)
        stored_origin = [0, 0, 0];
        for i = 1:length(thetas)
            T_total = T_total_bank{i} * M_intermediates{i};
            plotFrame_T(T_total, strcat('$\lbrace b_', string(i) , '\rbrace$'), 0.1, alpha);
            plotLink_p(stored_origin, T_total(1:3, 4), alpha/2);
            stored_origin = T_total(1:3, 4);
        end
        Tf = T_total_bank{end} * M;
        plotFrame_T(Tf, '$\lbrace f \rbrace$', 0.1, 1)
        plotLink_p(stored_origin, Tf(1:3, 4), alpha/2);
